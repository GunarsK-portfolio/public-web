      ...
      <div>
        ...
        <n-timeline>
          <n-timeline-item v-for="exp in experience" :key="exp.id">
            <n-card>
              <h3 style="font-size: 18px; font-weight: bold; margin-bottom: 4px">{{ exp.position }}</h3>
      ...
      <div>
        ...
        <n-grid :cols="2" :x-gap="16" :y-gap="16" responsive="screen">
          <n-grid-item v-for="cert in certifications" :key="cert.id">
            <n-card>
              <h3 style="font-size: 18px; font-weight: bold; margin-bottom: 4px">{{ cert.name }}</h3>
              <p style="margin: 4px 0">{{ cert.issuer }}</p>
              <p style="font-size: 14px; opacity: 0.7; margin: 4px 0">Issued: {{ cert.issue_date }}</p>
              <a v-if="cert.credential_url" :href="cert.credential_url" target="_blank" style="color: var(--n-color-target); text-decoration: none; margin-top: 8px; display: inline-block">
                View Credential →
              </a>
            </n-card>
          </n-grid-item>
